...
MatrixXd SCManager::makeScancontext( pcl::PointCloud<SCPointType> & _scan_down )
{
    ...
    std::cout << "makeScanContext\n";

    int num_pts_scan_down = _scan_down.points.size();
    
    std::cout << "num_pts_scan_down: " << num_pts_scan_down << std::endl;
    ...
    Eigen::MatrixXd dummy = MatrixXd::Ones(10, 60);

    std::cout << "dummy \n";

    // MatrixXd desc = static_cast<double>(NO_POINT) * MatrixXd::Ones(PC_NUM_RING, PC_NUM_SECTOR);
    // MatrixXd desc = MatrixXd::Constant(PC_NUM_RING, PC_NUM_SECTOR, static_cast<double>(NO_POINT));
    MatrixXd desc = MatrixXd::Constant(20, 60, -1000.0);

    std::cout << "desc matrix defined\n";
    ...
    for (int pt_idx = 0; pt_idx < num_pts_scan_down; pt_idx++)
    {
        ...
        pt.z = _scan_down.points[pt_idx].z; // naive adding is ok (all points should be > 0).
        ...
        std::cout << "azim_range: " << azim_range << std::endl;
        
        // azim_angle = xy2theta(pt.x, pt.y);
        azim_angle = std::atan2(pt.y, pt.x);

        std::cout << "azim_angle: " << azim_angle << std::endl;

        if(azim_angle < 0)
            azim_angle += 2*M_PI;
        
        azim_angle *= 180.0f/M_PI;
        ...
        std::cout << "ring_idx: " << ring_idx << std::endl;
        std::cout << "sctor_idx: " << sctor_idx << std::endl;
