...
// class ScanContext
    // public:
        ...
        std::pair<int, float> ScanContext::detectLoopClosureID(){
            ...
            if( this->time2resetTree() ){
                ...
                this->tree_.reset();
                this->tree_ = std::make_unique<InvKeyTree>(20/*PC_NUM_RING*/, invkeys_to_search_, 10/*leaf size*/);
            }

            this->buff_mtx.unlock();

            std::vector<size_t> candidate_idxs(10/*NUM_CANDIDATES_FROM_TREE*/);
            std::vector<float> candidate_dists(10/*NUM_CANDIDATES_FROM_TREE*/);

            nanoflann::KNNResultSet<float> knnsearch_result(10/*NUM_CANDIDATES_FROM_TREE*/);
            ...
            this->buff_mtx.lock();
            for(int id=0; id < candidate_idxs.size(); id++){
            ...
            if( min_dist < 0.13f/*SC_DIST_THRES*/){
                loop_id = nn_id;
            ...
            float yaw_diff = nn_align * 2.0*M_PI/static_cast<float>(60/*PC_NUM_SECTOR*/);
    ...
    // private:
        ...
        Eigen::MatrixXf ScanContext::makeScanContext(pcl::PointCloud<SCPointType>& scan){
            ...
            // Eigen::MatrixXf sc = Eigen::MatrixXf::Constant(20 /*PC_NUM_RING*/, 60/*PC_NUM_SECTOR*/, -1000.0f);
            Eigen::MatrixXf sc = -1000.0f * Eigen::MatrixXf::Ones(20 /*PC_NUM_RING*/, 60/*PC_NUM_SECTOR*/);
            for(int id=0; id < scan_size; id++){
                ...
                if(azim_range > 80.0/*PC_MAX_RADIUS*/)
                    continue;

                int ring_id   = std::max( std::min(20/*PC_NUM_RING*/,   int(ceil( (azim_range/80.0/*PC_MAX_RADIUS*/) * 20/*PC_NUM_RING*/ )) ), 1 );
                int sector_id = std::max( std::min(60/*PC_NUM_SECTOR*/, int(ceil( (azim_angle/(2.0*M_PI)) * 60/*PC_NUM_SECTOR*/ )) ), 1 );
        ...
        std::pair<float, int> ScanContext::distanceBtnScanContext(Eigen::MatrixXf &_sc1, Eigen::MatrixXf &_sc2){
            ...
            const int SEARCH_RADIUS = round( 0.5 * 0.1/*SEARCH_RATIO*/ * _sc1.cols() ); // a half of search range 
