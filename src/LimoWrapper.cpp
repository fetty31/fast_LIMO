    ...
    class LimoWrapper : public rclcpp::Node
    {
        ...
        /* ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// 
           ...
           void loadConfig(fast_limo::Config* config){
                ...
                this->declare_parameter("topics.input.lidar", "/lidar/points");    
                this->declare_parameter("topics.input.imu", "/sensors/imu");  

                this->declare_parameter("frames.world", "map");    
                this->declare_parameter("frames.body", "base_link");    

                this->declare_parameter("num_threads", 10);               
                this->declare_parameter("sensor_type", 1);    

                this->declare_parameter("debug",    true);           
                this->declare_parameter("verbose",  true);  

                this->declare_parameter("estimate_extrinsics",  true);           
                this->declare_parameter("time_offset",          true);           
                this->declare_parameter("end_of_sweep",         false);  

                this->declare_parameter("calibration.gravity_align",    true);           
                this->declare_parameter("calibration.accel",            true);           
                this->declare_parameter("calibration.gyro",             true);           
                this->declare_parameter("calibration.time",             3.0);    

                this->declare_parameter("extrinsics.imu.t", std::vector<float>{0.0f, 0.0f, 0.0f});           
                this->declare_parameter("extrinsics.imu.R", std::vector<float> (9, 0.0f));           
                this->declare_parameter("extrinsics.lidar.t", std::vector<float>{0.0f, 0.0f, 0.0f});           
                this->declare_parameter("extrinsics.lidar.R", std::vector<float> (9, 0.0f));
                this->declare_parameter("intrinsics.accel.bias", std::vector<float>{0.01f, 0.01f, 0.01f});           
                this->declare_parameter("intrinsics.accel.sm",  std::vector<float> (9, 0.0f));
                this->declare_parameter("intrinsics.gyro.bias", std::vector<float>{0.01f, 0.01f, 0.01f});
            
                this->declare_parameter("filters.cropBox.active", true);
                this->declare_parameter("filters.cropBox.box.min", std::vector<float>{-1.0f, -1.0f, -1.0f});
                this->declare_parameter("filters.cropBox.box.max", std::vector<float>{1.0f, 1.0f, 1.0f});

                this->declare_parameter("filters.voxelGrid.active", true);
                this->declare_parameter("filters.voxelGrid.leafSize", std::vector<float>{0.5f, 0.5f, 0.5f});

                this->declare_parameter("filters.minDistance.active", true);
                this->declare_parameter("filters.minDistance.value", 3.0);

                this->declare_parameter("filters.FoV.active", true);
                this->declare_parameter("filters.FoV.value", 180.0f);

                this->declare_parameter("filters.rateSampling.active", true);
                this->declare_parameter("filters.rateSampling.value", 2);

                this->declare_parameter("iKFoM.MAX_NUM_ITERS", 2);
                this->declare_parameter("iKFoM.MAX_NUM_MATCHES", 5000);
                this->declare_parameter("iKFoM.MAX_NUM_PC2MATCH", 10000);
                this->declare_parameter("iKFoM.LIMITS", 0.001);

                this->declare_parameter("iKFoM.Mapping.NUM_MATCH_POINTS", 5);
                this->declare_parameter("iKFoM.Mapping.MAX_DIST_PLANE", 2.0);
                this->declare_parameter("iKFoM.Mapping.PLANES_THRESHOLD", 5.0e-2);
                this->declare_parameter("iKFoM.Mapping.LocalMapping", false);

                this->declare_parameter("iKFoM.iKDTree.balance", 0.6f);
                this->declare_parameter("iKFoM.iKDTree.delete", 0.3f);
                this->declare_parameter("iKFoM.iKDTree.voxel", 0.2f);
                this->declare_parameter("iKFoM.iKDTree.bb_size", 2000.0);
                this->declare_parameter("iKFoM.iKDTree.bb_range", 500.0);

                this->declare_parameter("iKFoM.covariance.gyro", 0.0001);
                this->declare_parameter("iKFoM.covariance.accel", 0.01);
                this->declare_parameter("iKFoM.covariance.bias_gyro", 0.00001);
                this->declare_parameter("iKFoM.covariance.bias_accel", 0.0001);
