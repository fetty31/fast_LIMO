...
First, we need to install [KISS-Matcher](https://github.com/MIT-SPARK/KISS-Matcher). You can install it following the official instructions. 

Because we are currently working with cmake 3.16, we have a fork available with a downgraded cmake version (for installation in _Ubuntu 20.04_).

First you need to install [ROBIN](https://github.com/MIT-SPARK/ROBIN.git) downgrading its cmake version to 3.16.
...
Next, clone the Nano GICP pkg
```sh
git clone https://github.com/fetty31/nano_gicp.git
```

Finally, use default `catkin_make` or `catkin build` to build the code. By default it will compile under the `CMAKE_BUILD_TYPE="Release"` flag.
